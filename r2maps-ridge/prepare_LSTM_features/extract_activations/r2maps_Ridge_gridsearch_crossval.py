...
rootdir = "/neurospin/unicog/protocols/IRMf/LePetitPrince_Pallier_2018/"

matrices = []
def get_design_matrices(rootdir):
    for j in range(1, 10):
        data = pd.read_csv(os.path.join(rootdir, 'MRI/scripts-python/design-matrices/en/rms-wordrate-freq-bottomup/dmtx_{}.csv'.format(j)), header=None)
        dmtx = data[1:]
...
def compute_global_masker(rootdir, subjects):
    masks = [compute_epi_mask(glob.glob(os.path.join(rootdir, "MRI/fmri-data/en", "sub-%03d" % s, "func","*.nii"))) for s in subjects]
...
if __name__ == '__main__':
    ...
    # subjects = [59, 61, 62, 63]
    subjects = [57, 58, 59, 61, 62, 63, 64, 65, 66, 67, 68, 69, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81, 82, 83, 84, 86, 87, 88, 89, 91, 92, 93, 94, 95, 96, 97, 98, 99, 100, 101, 103, 104, 105, 106, 108, 109, 110, 113, 114, 115]
