...
def compute_crossvalidated_r2(fmri_runs, design_matrices, loglabel, logcsvwriter):
    ...
    reg = RidgeCV(alphas=[0.5, 0.75, 1.0, 3.0, 5.0])
    ...
    for train_outer, test_outer in outer_cv:
        ...
        for train_inner, test_inner in inner_cv:
            predictors = np.vstack(d for d in design_matrices)
            reg = RidgeCV(alphas=[0.5, 1.0])
            reg.fit(predictors, data)
            alpha = reg.alpha_
            print(alpha)
            fmri_data = np.vstack([train_outer])
...
if __name__ == '__main__':
   ...
   #   alphas = [0.001]
    #alpha = alphas[0]
        ...
        loglabel = subject
        logcsvwriter = csv.writer(open("test_with_alphas_05_075_1_3_5.log", "a+"))

        r2train, r2test = Parallel(n_jobs=-2)(compute_crossvalidated_r2(fmri_runs, matrices, loglabel, logcsvwriter))
